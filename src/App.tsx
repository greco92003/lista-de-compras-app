...
export default function App() {
  ...
  const addItem = () => {
    if (inputValue.trim() !== "") {
      setItems([...items, { id: Date.now(), text: capitalizeWords(inputValue), checked: false }]);
      setInputValue("");
      if (inputRef.current) {
  ...
  return (
    <div className={`min-h-screen p-4 ${darkMode ? "bg-black text-white" : "bg-white text-black"}`}>
      <div className="max-w-md mx-auto">
        ...
        {items.length > 0 && (
          <Button className={`mt-4 w-full ${darkMode ? "bg-red-600" : "bg-red-500"}`} onClick={clearList} variant="destructive">
            Limpar Lista
          </Button>
        )}
      </div>
